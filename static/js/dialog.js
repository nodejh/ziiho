...
$(document).ready(function(){
	loadappend('css',_CJG.dialog.css);
});
function dialog(param){
	...
	/*加载状态图片*/
	var loadSrc = _CJG.dialog.loadSrc;
	...
	switch(contentType.toLowerCase()){
		...
		case "iframe":
			...
			$.ajax({
				...
				success:function(html){
					_CJG.dialog.iframeId="dialog-content-iframe-"+getrandoms(4);
					$("#windown-content").html("<iframe id=\""+_CJG.dialog.iframeId+"\" src=\""+toUrl+"\" width=\"100%\" height=\""+parseInt(height)+"px"+"\" scrolling=\"auto\" frameborder=\"0\" marginheight=\"0\" marginwidth=\"0\"></iframe>");
					_CJG.dialog.iframe=window.frames[_CJG.dialog.iframeId];
				}
			});
		}
	/*设置标题*/	
