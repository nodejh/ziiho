...
<div class="clearfix ul-box" tab="yes">
	<ul class="ubox">
        ...
        <li class="clearfix is">
            <div class="clearfix inp">
            	<?php if(strlen(my_array_value('src', $qSubjectRs)) >= 1){ ?>
            	<p style="margin-bottom:5px;" id="srcbox"><img src="<?php prt($JMODEL->src($qSubjectRs['src'])); ?>" width="100" /><br/><button type="button" name="disabled-buttons" class="fbtn-ds" onclick="delSrcDo(this);" style="margin-top: 5px;">删除</button></p>
                <?php } ?>
...
function delSrcDo(_this){
	return _GESHAI.fsubmit(_this, "<?php prt(_g('cp')->uri('mod/job/ac/question/op/qsubject/f/delsrc/sortid/' . $sortid . '/questionid/' . $questionid)); ?>", {
		"start": function(){
			_GESHAI.disbtn("", true);
			window.top._GESHAI.dialog({isBg: false, isHeader: false, isFooter: false, data: "Loading..."});
		},
