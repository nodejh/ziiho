...
class class_admin_member extends class_member {
	
	/* 析构函数 */
	function __construct() {
		parent::__construct ();
	}
	function class_admin_member() {
		$this->__construct ();
	}
	
	/* 密码修改 */
	function admin_member_pwd_update($oldpassword, $password, $password2) {
		sleep ( 1 );
		/* 回调参数 */
		$callFunc = msg_func ();
		...
		/* 检查原始密码长度 */
		if (check_is_len ( $oldpassword, 1, 50 ) < 1) {
		...
		/* 原密码是否正确 */
		if (array_key_val ( 'password', $qrs ) !== md5_str ( $oldpassword, 2 )) {
			showmsg ( lang ( 'member:password_set_old_pwd_fail' ), NULL, $callFunc );
		}
		/* 如果原密码和新密码相同,则不需修改 */
		if (md5_str ( $oldpassword, 2 ) === md5_str ( $password, 2 )) {
		...
		/* 系统自动退出会员 */
		logout ();
		$url = url ( 'index', 'mod/admin/ac/entry/op/login' );
		showmsg ( lang ( 'admin:password_update_success' ), $url, $callFunc_b, array (
				'urlTarget' => '_top' 
		) );
	}
}
