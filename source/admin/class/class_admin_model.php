...
class class_admin_model {
	...
	function init() {
		...
		$this->lockscreen = _g ( 'module' )->trigger ( ':', 'lockscreen');
	}
	...
	function goindex($isprt = true) {
		$str = '<script language="javascript">if(window.top!=window.self){window.top.location.href="' . _g ( 'cp' )->uri () . '";}else{window.location.href="' . _g ( 'cp' )->uri () . '";}</script>';
		if ($isprt === true) {
			prt($str);
		} else {
			return $str;
	...
	function lockscreen_chk() {
		$dis = ( _g('cp')->lockscreen->is() ? 'block' : 'none' );
		$str = '<script language="javascript">window.top.lockscreenInput("'. $dis . '");</script>';
		prt ( $str );
	}
	/* page nav */
