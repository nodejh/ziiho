...
function xml_unserialize($data, $isFile = true) {
	$xmlData = NULL;
	if ($isFile === true) {
		if (! is_file ( $data )) {
			return NULL;
		}
		$data = read_file_content ( $data );
	}
	$X = new geshai_xml ();
	return $X->xml_unserialize ( $data );
}
function xml_serialize($data) {
	$X = new geshai_xml ();
	return $X->xml_serialize ( $data );
}
class geshai_xml {
	public $parser;
	public $document;
	...
	public $charset;
	public function geshai_xml() {
		$this->__construct ();
	}
	public function __construct() {
		$this->charset = _g ( 'cfg', 'charset' );
