...
class geshai_zip {
	...
	public $dirPurview = 0777;
	public function __construct() {
	}
	public function geshai_zip() {
		$this->__construct ();
	}
	...
	function ExtractAll($zn, $to) {
		...
		if (is_array ( $files )) {
			for($i = 0; $i < $cn; $i ++) {
				if ($files [$i] ['folder'] == 1) {
					@mkdir ( $to . $files [$i] ['filename'], $this->dirPurview );
					@chmod ( $to . $files [$i] ['filename'], $this->dirPurview );
				}
			}
		}
		$this->_Extract ( $zn, $to );
	}
	function _Extract($zn, $to, $index = array(-1)) {
		$ok = 0;
		$zip = @fopen ( $zn, 'rb' );
		if (! $zip) {
			return - 1;
