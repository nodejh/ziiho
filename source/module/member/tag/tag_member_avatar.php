...
if (! function_exists ( '_hook_tag_member_avatar' )) {
	function _hook_tag_member_avatar($module, $tagfunc, $matchs, $old_content = NULL) {
		/* 默认值 */
		$defaultVal = array (
				'return' => 'img' 
		);
		...
		/* ----------------------------------------------- */
		$_unTagContent = "<?php " . $s . $return . " = tag_member_avatar(" . arraytostr ( $tagAttr ) . "); ?>";
		return $_unTagContent;
	}
	/* 标签生成会员缩略图解析 */
	function tag_member_avatar($data) {
		/* 缩略图方式 */
		$typearr = array (
				yesno_val ( 'normal' ),
				yesno_val ( 'check' ) 
		...
		if ($data ['defaultshow'] == $isdefualtarr [0]) {
			if (str_len ( $data ['defaultsrc'] ) < 1) {
				$data ['defaultsrc'] = member_default_avatar ();
			}
		}
		...
		/* 获取原图 */
		$data ['src'] = member_avatar ( $data ['src'] );
		/* 原图地址 */
