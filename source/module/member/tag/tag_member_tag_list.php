...
if (! function_exists ( '_hook_tag_member_tag_list' )) {
	function _hook_tag_member_tag_list($module, $tagfunc, $matchs, $old_content = NULL) {
		/* 默认值 */
		$defaultVal = array (
				'return' => 'result',
	...
	$return = $tagAttr ['return'];
		...
		/* ----------------------------------------------- */
		$_unTagContent = "<?php " . $s . $sysVarName . " = tag_member_tag_list(" . arraytostr ( $tagAttr ) . "); ";
	...
	function tag_member_tag_list($data) {
		...
		/* 获取列表 */
		$db->from ( $tagObj->table_member_tag );
		if (str_len ( array_key_val ( 'where', $data ) ) < 1) {
			$db->where ( 'disabled', $disabled_normal );
		} else {
