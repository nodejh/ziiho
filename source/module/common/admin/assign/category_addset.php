...
if (check_nums ( $catid ) < 1) {
	showmsg ( lang ( 'common_adm:category_parentid_fail' ), $callback_url );
}
/* 上级分类是否存在 */
$category = $categoryObj->category_query ( 'catid', $catid );
if (check_is_array ( $category ) < 1) {
	showmsg ( lang ( 'common_adm:category_parentid_noexist' ), $callback_url );
}
$category = array (
...
$category_option = $categoryObj->category_list ( 'module', $category ['module'], $tpl, $catid );

/* 模板目录 */
$template_path = array_key_val ( 'templatepath', array_key_val ( 'web', $_G ) );
/* 提交url */
$submitUrl = url ( 'index', 'mod/common/ac/admin/op/category/ao/category_addsetdo' );
