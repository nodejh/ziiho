...
class class_common_badword_group extends class_model {
	public $table_common_badword_group = 'common_badword_group';
	
	/* 析构函数 */
	function __construct() {
		parent::__construct ();
	}
	function class_common_badword_group() {
		$this->__construct ();
	}
	
	/* 组查询 */
	function group_query($_key, $_val = NULL) {
		$this->db->from ( $this->table_common_badword_group );
	...
	function baseadd($listorder, $title) {
		...
		if (str_len ( $title ) < 1) {
			showmsg ( lang ( 'common:wordgroup_title_null' ), NULL, $callFunc );
		}
		...
		$result = $this->db->db_insert ( $this->table_common_badword_group, $data_arr );
		unset ( $data_arr );
		if ($result == $this->db->cw) {
	...
	function baseedit($cgroupid, $listorder, $title) {
		...
		for($i = 0; $i < array_number ( $cgroupid ); $i ++) {
			...
			$result = $this->db->db_update ( $this->table_common_badword_group, $data_arr, 'cgroupid', $cgroupid [$i] );
			unset ( $data_arr );
			if ($result == $this->db->cw) {
				showmsg ( lang ( 'global:dbexceptions' ), $url, $callFunc_b );
			}
	...
	function del($cgroupid) {
		...
		for($i = 0; $i < array_number ( $cgroupid ); $i ++) {
			...
			/* 删除 */
			$result = $this->db->db_delete ( $this->table_common_badword_group, 'cgroupid', $cgroupid [$i] );
			if ($result == $this->db->cw) {
				showmsg ( lang ( 'global:dbexceptions' ), $url, $callFunc_b );
			}
		}
