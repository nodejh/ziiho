...
if (! function_exists ( '_hook_tag_common_emotional_list' )) {
	function _hook_tag_common_emotional_list($module, $tagfunc, $matchs, $old_content = NULL) {
		/* 默认值 */
		$defaultVal = array (
				'return' => 'result',
	...
	$return = $tagAttr ['return'];
		...
		/* ----------------------------------------------- */
		$_unTagContent = "<?php " . $s . $sysVarName . " = tag_common_emotional_list(" . arraytostr ( $tagAttr ) . "); ";
	...
	function tag_common_emotional_list($data) {
		$EGObj = loader ( 'class:class_common_emotional', 'common', true, true );
		$db = $EGObj->db;
		/* 页码 */
		...
		/* 分类参数 */
		$emotgroupid = array_key_val ( 'emotgroupid', $data );
		if (check_nums ( $emotgroupid ) < 1) {
			return NULL;
		}
		$db->from ( $EGObj->table_common_emotional );
