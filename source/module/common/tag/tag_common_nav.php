...
if (! function_exists ( '_hook_tag_common_nav' )) {
	function _hook_tag_common_nav($module, $tagfunc, $matchs, $old_content = NULL) {
		/* 默认值 */
		$defaultVal = array (
				'return' => 'result',
	...
	$return = $tagAttr ['return'];
		...
		/* ----------------------------------------------- */
		$_unTagContent = "<?php " . $s . $sysVarName . " = tag_common_nav(" . arraytostr ( $tagAttr ) . "); ";
	...
	function tag_common_nav($data) {
		$nObj = loader ( 'class:class_common_nav', 'common', true, true );
		$db = $nObj->db;
		/* 页码 */
		...
		/* 导航id */
		$navid = array_key_val ( 'navid', $data );
		/* 导航类型 */
		$navtype = common_nav_type ( array_key_val ( 'navtype', $data, 'main' ) );
		/* 查询 */
		$db->from ( $nObj->table_common_nav );
		if (str_len ( array_key_val ( 'where', $data ) ) < 1) {
			if (check_nums ( $navid ) == 1) {
				$db->where ( 'navids', $navid );
			}
			if (! empty ( $navtype )) {
				$db->where ( 'navtype', $navtype );
			}
