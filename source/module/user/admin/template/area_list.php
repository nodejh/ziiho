...
<?php while($rs = $this->db->fetch_array($dataResult)){ ?>
    <tr class="trow-bline bg-hover-a">
        ...
        <td width="40%"><?php if($index >= 1){ ?><em style="padding-left:<?php prt($index * 16); ?>px; padding-right:5px;" class="tc-b"><?php if(is_array($this->find('parentid', $rs['areaid']))){ ?>├────<?php }else{ ?>└────<?php } ?></em><?php } ?><input type="text" name="aname[<?php prt($rs['areaid']); ?>]" class="fs-ts-180 <?php if($index < 1){ ?>t-fw<?php } ?>" value="<?php prt($rs['aname']); ?>" /><a class="ns fa-cd icon-add2" title="添加子分类" onclick="_GESHAI.redirect({'url':'<?php prt(_g('cp')->uri($writeUrlStr . '/parentid/' . $rs['areaid'])); ?>'});"></a></td>
