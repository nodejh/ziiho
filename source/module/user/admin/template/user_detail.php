            ...
            <div class="clearfix inp">
                <select multiple="multiple" name="csortid[]" class="fs-ts-h180">
                	...
                	<optgroup label="<?php prt(my_addslashes($CSortRs_v1['sname'])); ?>">
                        ...
                        <option value="<?php prt($CSortRs_v2['sortid']); ?>" <?php if(my_in_array($CSortRs_v2['sortid'], str2array(my_array_value('csortid', $CUserRs)))){ ?> selected="selected"<?php } ?> ><?php prt($CSortRs_v2['sname']); ?></option>
                        <?php } ?>
                    </optgroup>
                    <?php } ?>
                </select>
...
/* doing */
function fsdo(_this){
	return _GESHAI.fsubmit(_this, "<?php prt(_g('cp')->uri('mod/job/ac/cuser/op/detail_update')); ?>", {
