...
<div class="clearfix table-box">
	<form method="post" onsubmit="return false;" id="form_area">
...
function fsdo(_this, _t){
	var _thisForm = document.getElementById("form_area");
	if(_t == "delete"){
		...
		window.top._GESHAI.dialog({
				...
				"isCancelBtn": true,
				"okBtnFunc" : function(){
					return _GESHAI.fsubmit(_thisBtn, "<?php prt(_g('cp')->uri('mod/user/ac/area/op/delete')); ?>", {
						"start": function(){
							_GESHAI.disbtn("", true);
		...
		return _GESHAI.fsubmit(_this, "<?php prt(_g('cp')->uri('mod/user/ac/area/op/update')); ?>", {
			"start": function(){
				_GESHAI.disbtn("", true);
