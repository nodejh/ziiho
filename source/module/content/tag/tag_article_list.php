...
if (! function_exists ( '_hook_tag_article_list' )) {
	function _hook_tag_article_list($module, $tagfunc, $matchs, $old_content = NULL) {
		/* 默认值 */
		$defaultVal = array (
				'return' => 'result',
	...
	$return = $tagAttr ['return'];
		...
		/* ----------------------------------------------- */
		$_unTagContent = "<?php " . $s . $sysVarName . " = tag_article_list(" . arraytostr ( $tagAttr ) . "); ";
	...
	function tag_article_list($data) {
		$aObj = loader ( 'class:class_article', 'article', true, true );
		$db = $aObj->db;
		/* 页码 */
		$page = de_page_param ( array_key_val ( 'page', $data ) );
		/* 排序 */
		$orderby = de_db_orderby ( array_key_val ( 'orderby', $data ), 'aid,DESC' );
		/* 主题状态 */
