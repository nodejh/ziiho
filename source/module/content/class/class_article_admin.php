...
class class_article_admin extends class_article {
	
	/* 析构函数 */
	function __construct() {
		parent::__construct ();
	}
	function class_article_admin() {
		$this->__construct ();
	}
	
	/* 保存 */
	function save($statusname, $aid, $title, $tourl, $keywords, $origin, $originurl, $catid, $content, $description, $tid, $iscomment, $isslide, $isdescription, $views, $ctime, $template, $labelid, $attrid, $iconid, $abouts) {
		/* 回调参数 */
		...
		*/
		...
		/* 是否显示组图 */
		if (check_num ( $isslide ) < 1) {
			$isslide = array_key_val ( 'isslide', $ars, yesno_val ( 'check' ) );
		}
		/* 是否显示简介 */
		if (check_num ( $isdescription ) < 1) {
			$isdescription = array_key_val ( 'isdescription', $ars, yesno_val ( 'normal' ) );
		}
		/* 浏览数 */
		if (check_num ( $views ) < 1) {
			$views = array_key_val ( 'views', $ars, 0 );
		}
		/* 时间 */
		if (check_istime ( $ctime ) < 1) {
			$ctime = array_key_val ( 'ctime', $ars, $this->sys_time );
		} else {
			$ctime = str_strtotime ( $ctime );
		}
		/* 属性标签 */
		if (check_num ( $attrid ) < 1) {
			$attrid = array_key_val ( 'attrid', $ars, 0 );
		}
		/* 图标id */
		if (check_num ( $iconid ) < 1) {
			$iconid = array_key_val ( 'iconid', $ars, 0 );
		}
		unset ( $ars );
		/* 检查分类是否存在 */
		...
		if (check_is_array ( $categoryRs ) < 1) {
			if ($statusname != $statusdraft) {
				showmsg ( lang ( 'article:common_catid_noexist' ), NULL, $callFunc );
			} else {
				showmsg ( lang ( 'article:common_catid_noexist' ), NULL, $callFunc_c, array (
						'status' => 'no' 
				) );
			}
		}
		...
		/* 是否保存草稿操作 */
		if ($statusname != $statusdraft) {
			showmsg ( lang ( 'global:dbsuccess' ), $callbackUrl, $callFunc, array (
					'aid' => $aid 
			) );
		} else {
	...
	function update_category($aid, $catid) {
		...
		foreach ( $aid as $v ) {
			if (check_nums ( $v ) < 1) {
				showmsg ( lang ( 'global:param_ids_fail' ), NULL, $callFunc );
			}
		}
		/* 分类是否存在 */
