...
<div class="clearfix table-box">
	<form method="post" onsubmit="return false;" id="form_sort">
	<input type="hidden" name="id" value="" />
	<button type="button" name="disabled-buttons" id="btn_delete" class="dis-n">删除</button>
    <table class="tbox">
        <tr class="trow-bline">
            <td colspan="5"><a class="fa-cd icon-add" onclick="_GESHAI.redirect({'url': '<?php prt(_g('cp')->uri($writeUrlStr)); ?>'});">添加</a><a class="fa-cd icon-page-multi-add" onclick="_GESHAI.redirect({'url': '<?php prt(_g('cp')->uri('mod/job/ac/sort/op/multi_write')); ?>'});">批量添加</a>
            </td>
        </tr>
...
function fsdo(_this, _t){
	var _thisForm = document.getElementById("form_sort");
	if(_t == "delete"){
		...
		window.top._GESHAI.dialog({
				...
				"isCancelBtn": true,
				"okBtnFunc" : function(){
					return _GESHAI.fsubmit(_thisBtn, "<?php prt(_g('cp')->uri('mod/job/ac/sort/op/delete')); ?>", {
						"start": function(){
							_GESHAI.disbtn("", true);
		...
		return _GESHAI.fsubmit(_this, "<?php prt(_g('cp')->uri('mod/job/ac/sort/op/update')); ?>", {
			"start": function(){
				_GESHAI.disbtn("", true);
