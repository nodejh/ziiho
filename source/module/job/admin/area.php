...
switch (_get ( 'op' )) {
	case 'write' :
		...
		$gobackUrl = _g ( 'cp' )->uri ( 'mod/job/ac/area' . (_g ( 'validate' )->pnum ( $areaSub ['parentid'] ) ? ('/parentid/' . $areaSub ['parentid']) : '') );
		_g ( 'cp' )->set_template ( 'job', 'area_write' );
		break;
	case 'write_save' :
