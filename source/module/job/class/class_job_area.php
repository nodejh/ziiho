...
class class_job_area extends geshai_model {
	public $t_job_area = 'job_area';
	function __construct() {
		parent::__construct ();
	}
	function class_job_area() {
		$this->__construct ();
	}
	function find($k, $v = null) {
		$this->db->from ( $this->t_job_area );
		$this->db->where ( $k, $v );
		$this->db->select ();
	...
	function include_list($parentid = 0, $index = 0) {
		...
		$writeUrlStr = 'mod/job/ac/area/op/write';
		
		include (_g ( 'cp' )->get_template ( 'job', 'area_list' ));
	}
	...
	function option($areaSub, $parentid, $index = 0) {
		...
		$result = $this->db->get_list ();
		include _g ( 'cp' )->get_template ( 'job', 'area_option' );
	}
	...
	function include_cpos($parentid) {
		...
		$urlb = _g ( 'cp' )->uri ( 'mod/job/ac/area/parentid/' );
		
		include _g ( 'cp' )->get_template ( 'job', 'area_pos' );
	}
