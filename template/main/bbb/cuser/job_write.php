...
<?php include _g('template')->name('@', 'header_center', true); ?>
    ...
    <div class="cuser_y clearfix o-right">
        ...
        <h1 class="o-title">职位管理<a href="<?php prt(_g('uri')->su('user/ac/job')); ?>"><button class="o-button o-button-info o-title-tips">&laquo;返回</button></a></h1>
        ...
        <div class="form-item clearfix">
            ...
            <ul class="form-box">
                    ...
                    <div class="zplx">
                        <?php if($zplxPageData['total'] >= 1){ ?>
                            <?php while($zplxRs = _g('db')->result($zplxResult)){ ?>
                                <a href="javascript:;" select="zplx" checkbox-icon="true" <?php prt(my_in_array($zplxRs['zplxid'], $zplxData) ? 'checkbox-status="true" class="on o-zplx-a"' : 'class="o-zplx-a"');?>><input type="checkbox" name="zplxid[]" value="<?php prt($zplxRs['zplxid']); ?>" class="zp_ck o-input o-fix-select" <?php prt(my_in_array($zplxRs['zplxid'], $zplxData) ? 'checked="checked"' : '');?> /><em><?php prt($zplxRs['zname']); ?></em></a>
                            <?php } ?>
                        <?php }else{ ?>
                ...
                <li class="clearfix">
                    ...
                    <div class="inp o-inp-265"><select class="sel o-input o-fix-select" name="sortid" id="sort-data"><option value="0">==请选择==</option></select></div>
                </li>

                <li class="clearfix">
                    <div class="lab">测试时间：</div>
                    <div class="inp"><input type="text" name="examtime" class="it o-input" value="<?php prt(my_array_value('examtime', $jobSub)); ?>" /><span class="t-des">单位为“分钟”。如果为空，则为默认30分钟</span></div>
                </li>

                <li class="clearfix">
                    <div class="lab">招聘标题：</div>
                    <div class="inp"><input type="text" name="jname" class="it o-input" value="<?php prt(my_array_value('jname', $jobSub)); ?>" /></div>
                </li>

                <li class="clearfix">
                    <div class="lab">招聘人数：</div>
                    <div class="inp"><input type="text" name="pnum" class="it o-input" value="<?php prt(my_array_value('pnum', $jobSub)); ?>" /></div>
                </li>
                <li class="clearfix">
