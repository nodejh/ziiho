...
<?php include _g('template')->name('@', 'header_center', true); ?>
    ...
    <div class="cuser_center clearfix o-main" id="cuser_center">
        ...
        <div class="cuser_y clearfix o-right">
            ...
            <h1 class="o-title">职位管理<a href="<?php prt(_g('uri')->su('user/ac/job/op/write')); ?>">
                    <button class="o-button o-button-info o-title-tips">+发布职位</button>
                </a></h1>
                ...
                <table class="tbox">
                    ...
                    <?php while ($jRs = _g('db')->result($JJResult)): ?>
                        <tr class="trow-bline trow-hover">
                            ...
                            <td width="30%" class="ops"><a
                                    href="<?php prt(_g('uri')->su('user/ac/job/op/jexams/jobid/' . $jRs['jobid'])); ?>">测题管理</a><a
                                    href="<?php prt(_g('uri')->su('user/ac/job/op/esanswer/jobid/' . $jRs['jobid'])); ?>">答卷管理</a><a
                                    href="<?php prt(_g('uri')->su('user/ac/job/op/skill/jobid/' . $jRs['jobid'])); ?>">方案</a><a
                                    href="<?php prt(_g('uri')->su('user/ac/job/op/write/jobid/' . $jRs['jobid'])); ?>">修改</a><a
                                    href="javascript:;"
                                    onclick="cUserJobDelete(this, '<?php prt(_g('uri')->su('user/ac/job/op/delete')); ?>');"
                                    data-id="<?php prt($jRs['jobid']); ?>">删除</a></td>
                        </tr>
                    <?php endwhile; ?>
                </table>
            </div>
